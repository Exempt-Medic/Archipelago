...
def _can_do_theater_to_tunnels(state: CollectionState, world: "WitnessWorld") -> bool:
    ...
    tunnels_from_desert = (
        ...
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Main Island", "Desert Outside"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Desert Outside", "Desert Light Room"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Desert Light Room", "Desert Pond Room"])
    ...
    tunnels_from_caves = (
        any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Town", "Main Island"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Main Island", "Caves"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Caves", "Caves Path to Challenge"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Caves Path to Challenge", "Challenge"])
        and any(e.can_reach(state) for e in world.regio.two_way_entrance_register["Challenge", "Tunnels"])
    )

    return theater_from_town and tunnels_from_town or world.options.expect_prior_knowledge >= 2 and (tunnels_from_desert or tunnels_from_caves)
